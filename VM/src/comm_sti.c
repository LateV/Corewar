...
void		sti_reg_dir_dir(t_cor *cor, t_process *process)
{
		...
		(cor->mon == cor->cycles || cor->log == 1))
	{
		ft_printf("P% 5d | sti r%d %d %d\n       | -> store to %d +
			...
			process->arg3, process->arg2, process->arg3, process->arg2 +
			process->arg3, process->pc + ((process->arg2 +
			process->arg3) % IDX_MOD));
	}
	load_from_reg(cor, process, process->pc + ((process->arg2 +
		process->arg3) % IDX_MOD), process->arg1 - 1);
}
